...
import { FaArrowLeft, FaRobot, FaMicrophone, FaVolumeUp, FaPause } from 'react-icons/fa';
...
export default function StudentAssistant() {
  ...
  const handlePause = () => {
    ...
    // Reset both state variables to ensure consistency
    setPlayingIdx(null);
    setSpeaking(false);
  };
      ...
      <div className="assistant-container">
          ...
          <div className="assistant-header-content">
            <div className="assistant-header-text">
              <h1 className="assistant-title">
                <FaRobot className="assistant-title-icon" />
                eduAssistant
              </h1>
              <p className="assistant-subtitle">
                Your AI-powered learning companion. Ask me any doubt about your subjects and I'll help you understand better!
              </p>
            </div>
            <div className="assistant-header-image">
              <img src={botImage} alt="AI Assistant Bot" className="assistant-bot-image" />
            </div>
          </div>
        </div>
        ...
        <div className="assistant-controls">
          ...
          <div className="assistant-topic-row">
            <label className="assistant-label" htmlFor="topic-input">Topic (Optional)</label>
            <input
              id="topic-input"
              type="text"
              value={customTopic}
              ...
              placeholder="Enter a specific topic for more focused help"
              disabled={loading}
            />
          </div>
        </div>
        ...
        <div className="assistant-chat-section">
          <div className="assistant-chatbox">
            {messages.map((msg, idx) => (
              <div key={idx} className={`assistant-msg ${msg.sender}`}>
                ...
                {msg.sender === 'assistant' && (
                  <div className="audio-controls">
                    <button
                      className="audio-btn"
                      title={(playingIdx === idx || speaking === idx) ? "Pause audio" : "Listen to answer"}
                      onClick={() => (playingIdx === idx || speaking === idx) ? handlePause() : handleSpeak(msg.text, idx)}
                    >
                      {(playingIdx === idx || speaking === idx) ? 
                        <FaPause style={{color: '#553C9A'}} /> : 
                        <FaVolumeUp />
                      }
                    </button>
                  </div>
            ...
            {loading && (
              <div className="assistant-msg assistant-loading">AI is thinking...</div>
            )}
          </div>
        </div>
        ...
        <div className="assistant-input-section">
          <form onSubmit={handleSend} className="assistant-input-row">
            ...
            <button
              type="button"
              className={`assistant-mic-btn ${recognizing ? 'recognizing' : ''}`}
